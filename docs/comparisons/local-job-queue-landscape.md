...
| [Celery](https://github.com/celery/celery) | Application task queue | Distributed workers, retries with exponential backoff, time limits, scheduling | Jobs are Python functions. Needs a broker. A task killed mid-run is not re-run by default. | [Compare](/docs/comparisons/spoolctl-vs-celery/) |
