...
PUBLISHED_DOCS = [
    ...
    if path not in {DOCS / "README.md", DOCS / "changelog.md"}
]
...
class TestStructural(unittest.TestCase):
    ...
    def test_generated_pages_have_header(self):
        for path in GENERATED_PAGES:
            ...
            self.assertTrue(
                content.startswith("---\n"),
                f"{path.name} missing Astro frontmatter"
            )
            self.assertIn(
                "<!-- Generated by scripts/build_docs.py. Do not edit by hand. -->",
                content,
                f"{path.name} missing generator notice"
            )
    ...
    def test_published_docs_have_astro_frontmatter(self):
        required = {"title", "description", "bucket", "order"}
        for path in PUBLISHED_DOCS:
            content = path.read_text()
            match = re.match(r"^---\n(.*?)\n---\n", content, re.DOTALL)
            self.assertIsNotNone(
                match,
                f"{path.relative_to(REPO)} has no Astro frontmatter",
            )
            values = {
                line.split(":", 1)[0]
            ...
            self.assertTrue(
                required <= values,
                f"{path.relative_to(REPO)} has incomplete Astro frontmatter",
            )
            self.assertRegex(
                match.group(1),
                r"(?m)^bucket: (guides|concepts|project)$",
                f"{path.relative_to(REPO)} has an invalid bucket",
            )
            self.assertRegex(
                match.group(1),
                r"(?m)^order: \d+$",
                f"{path.relative_to(REPO)} has an invalid order",
            )

    def test_published_docs_do_not_link_to_markdown_pages(self):
        for path in PUBLISHED_DOCS:
            content = path.read_text()
            self.assertNotRegex(
                content,
                r"\]\([^)]*\.md(?:#[^)]*)?\)",
                f"{path.relative_to(REPO)} has a Markdown page link",
            )
